...
function render() {
  ...
  if (currentScreen === 'solo') return renderSolo(soloMovies, navigate);
  ...
  switch (room.status) {
    case 'lobby': currentScreen = 'lobby'; renderLobby(room, navigate); break;
    case 'loading': renderLoading(room); break;
    case 'swiping': renderSwipe(room); break;
    case 'results': renderResults(room, navigate); break;
    default: renderLobby(room, navigate);
  }
}

export async function startSolo(filters) {
  const response = await fetch('/api/discover', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(filters) });
  if (!response.ok) throw new Error('discover_failed');
  soloMovies = await response.json();
  if (!soloMovies.length) throw new Error('empty_deck');
  appState.mode = 'solo';
  navigate('solo');
}
...
async function init() {
  ...
  onRoomState((room) => {
    ...
    if (room.filters && room.isHost && currentScreen !== 'filters') appState.filtersDraft = { ...room.filters };
    render();
  });
  const params = new URLSearchParams(window.location.search);
  const roomFromQr = params.get('room');
  if (roomFromQr) { appState.prefillRoomCode = roomFromQr.toUpperCase().slice(0, 4); navigate('join'); return; }
