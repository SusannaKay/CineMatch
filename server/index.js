...
function emitRoom(room) { for (const [socketId] of room.players) { const sock = io.sockets.sockets.get(socketId); if (sock) sock.emit('room:state', room.toClient(socketId)); } }
...
io.on('connection', (socket) => {
  socket.on('room:create', ({ name }) => { const displayName = String(name || 'Host').trim().slice(0, 20) || 'Host'; const room = rooms.createRoom(socket.id); room.addPlayer(socket.id, displayName); rooms.bindSocket(socket.id, room.id); socket.join(room.id); socket.emit('room:state', room.toClient(socket.id)); });
  socket.on('room:join', ({ code, name }) => {
    const room = rooms.getRoom(code); const displayName = String(name || 'Ospite').trim().slice(0, 20) || 'Ospite';
    ...
    room.addPlayer(socket.id, displayName); rooms.bindSocket(socket.id, room.id); socket.join(room.id); emitRoom(room);
  });
  socket.on('room:leave', () => handleDisconnect(socket));
  socket.on('room:set-filters', ({ filters }) => { const room = rooms.getRoomBySocket(socket.id); if (!room || !room.isHost(socket.id) || room.status !== 'lobby') return; room.filters = filters; emitRoom(room); });
  socket.on('room:start', async () => {
    const room = rooms.getRoomBySocket(socket.id); if (!room || !room.isHost(socket.id)) return;
  ...
  function handleDisconnect(sock) {
    ...
    if (!room.playerCount()) { clearVoteTimer(room); rooms.deleteRoom(room.id); return; }
    if (room.status === 'swiping' && room.currentMovie()) { const status = room.votesForCurrent(); if (status.voted >= room.playerCount()) advanceOrFinish(room); else emitRoom(room); } else emitRoom(room);
  }
});
